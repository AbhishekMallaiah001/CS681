...
public class PVIParallelStreams {
    ...
    public static void main(String args[]) throws IOException {
        ...
        try (Stream<String> lines = Files.lines(path)) {
            ...
            List stateMA = pviData.parallelStream().filter((i) -> i.get(STATEINDEX).contains("Massachusetts")).collect(Collectors.toList());

            List suffolk = pviData.parallelStream().filter((i) -> i.get(COUNTYINDEX).contains("Suffolk")).collect(Collectors.toList()).get(0);
            System.out.println("\n==> Total cases found in Suffolk county:   " + suffolk.get(CASEINDEX));

            String casesInMA = pviData.parallelStream().filter((i) -> i.get(STATEINDEX).contains("Massachusetts"))
            ...
            System.out.println("\n==> MEAN of the total cases found in Massachussetts State:     " + Integer.parseInt(casesInMA) / stateMA.size());
